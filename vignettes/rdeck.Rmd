...
to define the centre of each rendered point, and the `gender` column to change the point
...
The scatterplot points are _scaled_ by the gender categories found in the data (which in this
dataset are `1` and `2`); this colour scale generates a categorical legend.
...
Hovered points will become brighter (keeping a similar colour scale as fill colour) and will
render a tooltip containing the gender category from the data.
...
rdeck(
  ...
  add_scatterplot_layer(
    ...
    # a categorical colour scale, using the gender column and a cividis colour palette
...
In the following example, we group all points by gender so that we can highlight all points of
a given gender at once.
...
and the tooltip (to include the new column, `count`)
...
the radius by the count of `gender`.
...
The only rule in creating a layer function is that it _must_ return the map to make it chainable.

In the following example, we parameterise the data, fill palette, highlight palette, and
`get_radius` (which will require curl-curly) for our original scatterplot layer.
